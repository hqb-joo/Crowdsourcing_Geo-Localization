...
def train(start_epoch=0):
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(start_epoch, number_of_epoch):
            ...
            while True:
                # train
                batch_sat_polar, batch_sat, batch_crd, batch_utm= input_data.next_pair_batch(batch_size)
                ...
                feed_dict = {crd_x: batch_crd, polar_sat_x: batch_sat_polar,
                ...
                iter += 1
                del batch_sat_polar, batch_sat, batch_crd, batch_utm
            ...
            sat_descriptor = np.reshape(sat_global_matrix[:, :, :c_width, :], [-1, c_height * c_width * c_channel])
            ...
            with open('../Result/' + data_type + '/FOV_polar_' + str(polar) + '_' + str(network_type) + str(mat_type)+ '.txt', 'a+') as file:
